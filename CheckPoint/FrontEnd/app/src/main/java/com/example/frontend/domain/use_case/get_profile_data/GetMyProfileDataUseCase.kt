...
class GetMyProfileDataUseCase @Inject constructor(
    ...
    operator fun invoke(token:String) : Flow<Resource<ProfileData>> = flow{
        try{
            ...
            val followers = repository.getMyFollowers(token).map{
                it.toUser()
            }
            val following = repository.getMyFollowing(token).map{
                it.toUser()
            }
            ...
            val profileData = ProfileData(followers, following, followersCount, followingCount, postCount)
